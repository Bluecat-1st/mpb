# Note:
This is not a serious project, don't expect compatibilty to run `mpb` on other devices, in fact, it would be perfered to not run `mpb` on public servers[1].
...
Also, I'm likly not going to update this repository very often as new features/capibilities of `mpb` are going to be slow.
...
[1] Although, if you **really** want to, at least change the UUID so other's don't think it is me runninh `mpb` as I don't want to private or delete this repository because someone wanted to do something really dumb, but I will very much do so if I have to. **Listen to admins!**