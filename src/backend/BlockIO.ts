...
export class BlockIO {
    static readMain(buf:DataStream, type:string, ver:byte):Record<string,any>|null{
		if(type == "GenericCrafter" || type == "Separator" || type == "HeatProducer" || type == "HeatCrafter"){
			...
			let payload = TypeIO.readPayload(buf) as Record<string,any>;
			let progress;
			if(type == "UnitFactory"){
				progress = buf.getFloat()
			} else if(ver >= 1){
    ...
    static readPayloadSeq(buf:DataStream){
    ...
    static writeItemsM(buf:DataStream, items:any, legacy:boolean) {
	    let count = Object.keys(items).length;
	    legacy ? buf.put(count as byte) : buf.putShort(count as short);

	    for (let itemstring in items) {
	    	let itemid = parseInt(itemstring);
	    	legacy ? buf.put(itemid as byte) : buf.putShort(itemid as short);
	        buf.putInt(items[itemid]);
	    }
	}
	static writeLiquidsM(buf:DataStream, liquids:any, legacy = false) {
	    let count = Object.keys(liquids).length;
	    legacy ? buf.put(count as byte) : buf.putShort(count as short)

	    for (let liqname in liquids) {
	    	let liqid = parseInt(liqname);
	        legacy ? buf.put(liqid as byte) : buf.putShort(liqid as short);
	        buf.putInt(liquids[liqid]);
	    }
	}

	static writePowerM(buf:DataStream, power:[int[],float]) {
	    let [links, status] = power;
	    buf.putShort(links.length as short);
    ...
    static getModuleBitmask(id:string){
		return ((blocksParams as Record<string,any>)[id].hasItems ? 1 : 0 | ((blocksParams as Record<string,any>)[id].hasPower ? 2 : 0) | ((blocksParams as Record<string,any>)[id].hasLiquids ? 4 : 0)) | 8
	}
    static readItemsM(buf:DataStream, legacy:boolean){
		let count = legacy ? buf.get() : buf.getShort()

		let items:Record<byte|short,int> = {}
	...
	static readLiquidsM(buf:DataStream, legacy:boolean){
		let count = legacy ? buf.get() : buf.getShort()

		let liquids:Record<byte|short,int> = {}
		for(let j = 0; j < count; j++){
	...
	static readPowerM(buf:DataStream, legacy:boolean){
		let amount = buf.getShort();
		let links: int[] = []
		...
		return [links, status] as [int[], float];
	}
    static readBase(buf:DataStream, id:string){
    ...
    static readAll(buf:DataStream, id:string, type:string, ver:byte){
		...
		return [base, main] as [ReturnType<typeof BlockIO.readBase>, ReturnType<typeof BlockIO.readMain>];
	}
	static writePayloadSeq(buf:DataStream, ent:{type:byte,entr:short,count:int}[]) {
	    buf.putShort(/*-*/ent.length as short);
	    for (let e of ent) {
	        buf.put(e.type);
	        buf.putShort(e.entr);
	        buf.putInt(e.count);
	...
	static writeMain(buf:DataStream, type:string, ver:number, data:any) {
		if (type == "GenericCrafter" || type == "Separator" || type == "HeatProducer" || type == "HeatCrafter") {
			...
			for (let i = 0; i < data.map.length; i++) {
				if (ver == 0) {
					...
					buf.putInt(val as int);
				} else {
					buf.putShort(data.map[i].id);
					buf.put((data.map[i].x * 127) as byte);
					buf.put((data.map[i].y * 255 - 128) as byte);
				}
			}
		} else if (type == "StackConveyor") {
